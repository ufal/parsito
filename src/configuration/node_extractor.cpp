...
bool node_extractor::create(string_piece description, string& error) {
  string literal_stack = "stack";
  string literal_buffer = "buffer";
  string literal_parent = "parent";
  string literal_child = "child";
  ...
  for (auto&& line : lines) {
    ...
    start_t start;
    if (literal_stack.compare(0, literal_stack.size(), words[0].str, words[0].len) == 0)
      start = STACK;
    else if (literal_buffer.compare(0, literal_buffer.size(), words[0].str, words[0].len) == 0)
      start = BUFFER;
    else
    ...
    for (size_t i = 1; i < parts.size(); i++) {
      ...
      if (literal_parent.compare(0, literal_parent.size(), words[0].str, words[0].len) == 0) {
        if (words.size() != 1)
      ...
      } else if (literal_child.compare(0, literal_child.size(), words[0].str, words[0].len) == 0) {
        if (words.size() != 2)
